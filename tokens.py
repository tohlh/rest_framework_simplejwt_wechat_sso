    ...
    def token_backend(self):
        if self._token_backend is None:
            self._token_backend = import_string(
                "rest_framework_simplejwt_wechat_sso.state.token_backend"
            )
...
class BlacklistMixin:
    """
    If the `rest_framework_simplejwt_wechat_sso.token_blacklist` app was configured to be
    ...
    if "rest_framework_simplejwt_wechat_sso.token_blacklist" in settings.INSTALLED_APPS:
